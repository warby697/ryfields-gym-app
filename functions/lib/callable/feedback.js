...
export const submitSessionFeedback = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (!parsed.success)
        throw new HttpsError('invalid-argument', 'Feedback is invalid.');
    ...
    if (member.get('authUid') !== request.auth.uid && !staff)
        throw new HttpsError('permission-denied', 'You cannot leave feedback for this member.');
    let visitAt = null;
    if (parsed.data.visitId) {
        const visit = await db.collection('visits').doc(parsed.data.visitId).get();
        if (!visit.exists || visit.get('memberId') !== parsed.data.memberId)
            throw new HttpsError('not-found', 'We couldn’t find that gym visit.');
