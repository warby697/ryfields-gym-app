...
function parse(schema, data) {
    ...
    if (!result.success)
        throw new HttpsError('invalid-argument', 'The supplied cash-payment details are invalid.');
    return result.data;
}
const gbp = (minor) => `£${(minor / 100).toFixed(2)}`;
const asDate = (v) => v instanceof Timestamp ? v.toDate() : v instanceof Date ? v : null;
...
export const setCashSchedule = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (Number.isNaN(nextDue.getTime()))
        throw new HttpsError('invalid-argument', 'The next due date is invalid.');
...
export const recordCashPayment = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (typeof amountMinor !== 'number')
        throw new HttpsError('failed-precondition', 'Set a cash amount for this member first.');
    const paidAt = input.paidAt ? new Date(input.paidAt) : new Date();
    if (Number.isNaN(paidAt.getTime()))
        throw new HttpsError('invalid-argument', 'The payment date is invalid.');
