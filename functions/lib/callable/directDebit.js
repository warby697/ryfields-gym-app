...
async function gcRequest(path, opts = {}) {
    const token = process.env.GOCARDLESS_ACCESS_TOKEN;
    if (!token)
        throw new HttpsError('failed-precondition', 'Direct Debit is not available just yet. Please contact the gym.');
    ...
    if (!res.ok)
        throw new HttpsError('internal', json?.error?.message || 'GoCardless request failed.');
...
export const startDirectDebit = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (snap.get('authUid') !== request.auth.uid && !['staff', 'admin'].includes(String(role)))
        throw new HttpsError('permission-denied', 'You cannot set up payment for this member.');
    ...
    const billing = await gcRequest('/billing_requests', { method: 'POST', idempotencyKey: idem, body: { billing_requests: { mandate_request: { scheme: 'bacs' }, metadata: { member_id: memberRef.id, membership_type_id: typeId } } } });
    // Prefill the hosted page with what we already hold so the member isn't retyping their signup details.
    const prefilled = { country_code: 'GB' };
    ...
    if (given)
        prefilled.given_name = given;
    if (family)
        prefilled.family_name = family;
    if (address)
        prefilled.address_line1 = address;
    if (postcode)
        prefilled.postal_code = postcode;
    const flow = await gcRequest('/billing_request_flows', { method: 'POST', idempotencyKey: `${idem}-flow`, body: { billing_request_flows: { redirect_uri: `${appBase}/payment/return`, exit_uri: `${appBase}/`, prefilled_customer: prefilled, links: { billing_request: billing.id } } } });
